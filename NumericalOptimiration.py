...
###############################################################################
def combined_objective_k_r1_r2(vars_, R21_fixed, R23_fixed, data12, data32, x_grid):
...
def main():
    ...
    R21_candidates = np.round(np.arange(0.5, 2.51, 0.25), 2)  # e.g. 0.50, 0.75, 1.00, ...
    R23_candidates = np.round(np.arange(0.5, 5.01, 0.5), 2)  # e.g. 0.50, 1.00, 1.50, ...
    ...
    for R21_fixed in R21_candidates:
        for R23_fixed in R23_candidates:
            ...
            if res.success and res.fun < best_obj:
                ...
                best_solution = (res.x[0], res.x[1], res.x[2], res.x[3], res.x[4], R21_fixed, R23_fixed)
