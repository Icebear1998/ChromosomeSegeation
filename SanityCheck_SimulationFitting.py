...
def plot_hist_with_kde_overlays(datasets_true: Dict, datasets_rec: Dict, out_dir: str, tag: str):
	"""For each strain and pair, plot empirical histogram (true) and KDE overlays (true vs recovered)."""
	...
	for strain in ['wildtype', 'threshold', 'degrate', 'degrateAPC']:
		for key, title_pair in [('delta_t12', 'T1-T2'), ('delta_t32', 'T3-T2')]:
			...
			# Recovered KDE (if available)
			if datasets_rec is not None and strain in datasets_rec:
				rec_vals = datasets_rec[strain][key]
				y_rec = kde_curve(rec_vals, x_grid)
				plt.plot(x_grid, y_rec, color='C0', lw=2, label='Recovered KDE')
...
def main():
	...
	print("=" * 60)
	# Config
	mechanism = 'time_varying_k_burst_onion'  # Change to any supported mechanism
	num_simulations_gen = 400  # per strain for synthetic generation
	num_simulations_fit = 150  # per evaluation in optimization (SimulationOptimization_join defaults will be used)
	max_time = 600
	...
	# Step 2: Run simulation-based optimization to recover params
	print("\nRunning simulation-based optimization...")
	results = run_sim_optimization(
		mechanism=mechanism,
	...
	recovered = results['params'] if results and results.get('success') else None
	if recovered is None:
		print("Optimization failed; skipping plots.")
		return
	# Enrich recovered with derived n/N for plotting comparisons
	recovered_full = derive_wildtype_params_from_ratios(recovered)

	# Step 3: Re-simulate from recovered parameters for KDE overlays
	print("\nRe-simulating from recovered parameters for plotting...")
	rec_datasets = simulate_dataset(mechanism, recovered_full, num_simulations_gen, max_time)

	# Step 4: Plots
	print("\nGenerating plots...")
	plot_hist_with_kde_overlays(synthetic_datasets, rec_datasets, out_dir, tag=mechanism)
	plot_param_bars(true_full, recovered_full, out_dir, method_tag=mechanism)
	...
	print("Recovered (subset):", {k: round(recovered_full[k], 3) for k in ['n2','N2','r21','r23','R21','R23','k_1','k_max'] if k in recovered_full})
	print("Done.")
